# bgm-montage v1.4.6 使用说明

## Local Library Mode

`--source-provider local-library --local-library-dir PATH` 完全禁止素材联网搜索/下载。每次正式剪辑先递归轻扫目录，将持久索引写到 `<cache-dir>/local-library/libraries/<library-id>/library_index.json`，不修改素材库。首次、文件变化或 v1.4.2 索引迁移时，对每条素材执行 ffprobe、一次有界内容指纹和 6 帧轻量视觉画像；未变化条目直接复用 JSON，删除文件会从索引移除。

```powershell
& $Python (Join-Path $SkillRoot "scripts\bgm_montage.py") `
  --source-provider local-library `
  --local-library-dir "E:\media\冰岛" `
  --bgm "D:\music\track.mp3" --theme "冰岛冰川、瀑布与火山" `
  --duration 30 --ratio 16:9 --output-dir "D:\renders"
```

多阶段选片不会重新深解码全库：第一阶段用全库持久轻量画像（场景/主体、景别、运动、HSV、清晰度、曝光和感知哈希）、BGM 需求、已有深度特征与历史次数粗排；第二阶段只保留 `min(64, max(16, assets×3, slots×2))` 个高相关候选。候选没有有效深度缓存时才执行现有 48 帧分析，再交给已有片段窗口、BGM 槽位、连续性与多样性评分。`sync.light_analyzed`、`light_cache_hits`、`light_cache_hit_rate`、`deep_analyzed` 与 `selection.deep_analysis_during_selection` 给出本轮真实计算量。新增 100 条会让 100 条全部获得轻量画像，但只有 Top-K 进入深析。成功成片后，实际使用区间与 run ID 按内容指纹回写持久索引；移动/改名保留历史，同路径替换内容则重置历史。同步、深析合并和历史回写均使用跨进程锁与原子 JSON 替换。

## YouTube-first acquisition

The unified entry defaults to `--source-provider youtube-first`. Explicit `youtube`, `pixabay`, and network-free `local-library` modes remain available. v1.4.6 retains the v1.4 scene/grammar work, v1.4.1 Agent review, persistent whole-library lightweight visual recognition, and concurrent local-index updates, adding frame-safe planned-shot coverage and visual-impact-aware climax evidence. `yt-dlp` is only needed by YouTube modes.
...
For a reviewed long-form YouTube candidate, repeat `--youtube-source-window VIDEO_ID=START-END` to request absolute source seconds. Repeating the option with the same ID creates multiple independent curated clips, for example `ID=10-18` and `ID=42-51`; each interval has its own cache filename, manifest identity, and selected asset ID. The explicit intervals override the automatic 38-second long-video window, are validated before acquisition, enter the resume fingerprint, and only affect that ID if it is actually discovered and downloaded.
...
```powershell
$Package = "E:\资料\skills\造球计划\bgm-montage-v1.4.6-runtime.zip"
$GlobalSkill = Join-Path $env:USERPROFILE ".codex\skills\bgm-montage"
$Stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$Stage = Join-Path ([IO.Path]::GetTempPath()) "bgm-montage-v1.4.6-$Stamp"
...
- `--source-provider`：默认 `youtube-first`；也可显式选择 `youtube`、`pixabay` 或完全离线的 `local-library`。
- `--local-library-dir`：`local-library` 模式的只读视频素材根目录；索引写到 `--cache-dir`，不写入素材库。
- `--search-query`：显式优先查询，可重复；它只提高优先级，不关闭动态查询扩展。
- `--exclude-youtube-id ID`：审片后排除指定 YouTube ID，可重复。
- `--youtube-source-window VIDEO_ID=START-END`：指定某个 YouTube 候选的绝对源时间窗；同一 ID 可重复指定多个独立区间。
...
- `--agent-visual-review`：默认 `required`；程序化 QA 通过后等待视觉 Agent 实际查看证据帧并写入结构化结果。兼容旧的无人值守运行时可显式传 `off`。
...
未给 `--run-id` 时会生成 UTC 时间戳和随机后缀。运行目录存在且未传 `--resume-run` 时直接失败；不会静默覆盖历史成片。`--resume-run` 会校验 BGM 指纹、主题、时长、比例、素材/缓存路径和主要约束，然后复用已成功写出的阶段产物。未完成 attempt 的 MP4 仅在同目录 `render_provenance.json` 同时匹配 renderer SHA-256、规范化编辑计划摘要、BGM SHA-256、实际风格摘要、比例、帧率及视频大小/全文件 SHA-256 时复用；任一字段缺失、损坏或不匹配都会调用既有 `.part.mp4` 原子路径重渲，失败时保留旧 MP4 但不保留可信 provenance。成功重渲会归档旧 Agent 审片请求/结果并重新等待审片。已经成功、成片仍存在且 renderer 一致的运行保持不可变并直接返回既有报告；renderer 已变化时要求新 `run-id`。
...
- `attempts/attempt_XX/render_provenance.json`：绑定该 attempt 的渲染输入与 MP4 哈希，控制断点续跑是否允许复用视频。
...
- `local_library_index`（本地库模式）：全库文件签名、内容采样指纹、轻量视觉画像、深度分析结果、可用片段、使用历史和最近同步统计的持久索引。
...
- `agent_visual_review_request.json` / `agent_visual_review.json`：视觉 Agent 的待审目标与结构化状态、原因、时间戳、置信度、建议动作及逐帧观察。
...
检查帧覆盖开头、结尾、段落边界、乐句边界、drop、surge、climax、hard stop、计划切点前后和稳定种子的随机时间点，并同时生成 `visual_review.json` / `visual_review.md`；渲染先把全部镜头量化到统一帧网格，避免逐段帧率取整累积后截短尾镜。

程序化 QA 通过后，统一入口默认写出 `agent_visual_review_request.json`，返回 `status=awaiting_agent_visual_review` 和 CLI 退出码 `3`。执行该 Skill 的 Agent 必须逐一用图像工具打开 `review_targets[*].frame_path`，按 `response_template` 把逐帧观察和总体 `pass` / `warning` / `fail` 写入请求指定的 `agent_visual_review.json`，然后用原参数、原 `--run-id` 与 `--resume-run` 继续。结果会校验 attempt、成片 SHA-256、模型身份、实际图像查看声明、全部证据覆盖、时间戳、原因、置信度和建议动作；缺项不能伪装为通过。

程序化 QA 或 Agent Visual Review 的 `fail` 都进入同一个有界返工循环。`replace_shot` 所指镜头源会从下一次尝试排除，其余失败至少使用新 seed 重新选镜、重排并重渲染；`warning` 进入最终报告但不阻断。全部尝试失败时 `run_report.json` 保留诊断，不交付成片，也不提交素材使用历史。程序化 QA 负责可确定的媒体/时间线问题，Agent Review 补充事件响应、切点关系、构图连续性和明显视觉瑕疵；二者互不替代。
...
以下测试与打包命令只在唯一开发源码或 Development 包中执行；Runtime 包只用于安装和运行，不携带打包器。
...
构建精简 Runtime Skill 包：

```powershell
& $Python (Join-Path $SkillRoot "scripts\package_skill.py") `
  --version 1.4.6 --profile runtime `
  --output "E:\资料\skills\造球计划\bgm-montage-v1.4.6-runtime.zip"
```
...
```powershell
& $Python (Join-Path $SkillRoot "scripts\package_skill.py") `
  --version 1.4.6 --profile development `
  --output "E:\资料\skills\造球计划\bgm-montage-v1.4.6-development.zip"
```
...
```powershell
Expand-Archive -LiteralPath "D:\downloads\bgm-montage-v1.4.6-runtime.zip" `
  -DestinationPath "D:\my-montage-project"
```

Linux/Docker 可使用 `unzip bgm-montage-v1.4.6-runtime.zip -d /workspace/project` 或 `python -m zipfile -e ...`。这只表示 ZIP 路径兼容；本版本不宣称已完成 Linux/Docker 的依赖、模型、FFmpeg 或剪映现场运行验证。

## 12. 能力状态

当前 Runtime 包含确定性音乐时间线、BGM 驱动选片、全库轻量视觉索引、有界深析、增量缓存、内容身份、并发事务、渲染、程序化 QA 与视觉 Agent 返工闭环。
